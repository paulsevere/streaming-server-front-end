export const socketMiddleware = socket => () => next => action => {
  if (testProps(action, 'meta.socket.channel')) {
    console.log(action.meta.socket.room_id)
    socket.emit(action.meta.socket.channel, {action,room_id:action.meta.socket.room_id})
  }
  return next(action)
}
...
export const handleRouteChange = socket=>()=>next=>action=>{
  if(action.type.indexOf('@@router') > -1){
    socket.emit('NEW_ROOM', {room_id:action.payload.pathname.slice(1)})
  }
